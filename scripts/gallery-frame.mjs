...
try {
  ...
  check('the grid drew a frame layer over each photo', grid.shots === 4 && grid.frames === 4, JSON.stringify(grid));
  check('the frame layer has real size', grid.frameShown, JSON.stringify(grid));

  await page.click('.gal-shot');
  await wait(500);
  const big = await page.evaluate(() => {
    const img = document.querySelector('.gal-big-pic > img');
