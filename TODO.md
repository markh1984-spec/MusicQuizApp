...
1. **The arcade board does not name the game.**
   *Fix:* `lobby-board.js:54` prints a bare *"Top scores"*. The board is already
   told which game it is — print it: *"Top scores · Maze Mouth"*. One line, plus
   a screenshot at 1280 because it is on the projector.

2. **No player id on the archived leaderboard.**
...
3. **The Console shelf has no route to the Workshop.**
...
4. **Gigs never says a prize was taken.**
...
5. **A pin's ORDER is thrown away.**
...
6. **Launch does not open the big screen.**
...
7. **The demo prize card is hardcoded.**
...
8. **The calendar export calls every night a quiz.**
...
9. **The upload wording does not mention the public gallery.**
...
10. **A bingo pack cannot join the running order.**
    *Fix:* the guard is one line, twice — `console-tonight.js:1834` and `:1890`
    ...
    launch it. It needs `{ kind, packId }` slots and the launch to walk them.
    **Decision: is this wanted at all, given a SHOW already plays a quiz and
    then a bingo game?** If shows cover it, delete this entry instead.

### The unblocker

11. **A NIGHT AND A BOOKING ARE STILL TWO THINGS.**
...
13. **Email has one caller.** Transport is built, two providers, reset wired.
    *Fix:* invoices next. **Decision: does it SEND, or draft for him to send?**
    CLAUDE.md's rule is the app prepares and the human sends.
14. **"Add a past gig"** — a write route, an explicit `playerCount`, an
    `enteredByHand` flag, and every reader taught to respect it.
15. **The automatic gallery publish** — gate, route and caller are built; the
    trigger is not. Needs 11 for a real end time.
16. **An advert QR that COUNTS** — the offer page served from this app, so
    opens can be counted. The count is the feature, not the discount.
17. **The post-night report** — every number it needs already exists. Needs 11.
