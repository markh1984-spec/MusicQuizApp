...
**AND SPLIT AGAIN ON 2 SEPTEMBER 2026, the same way and for the same reason.**
The PUBLIC PAGE had become its own story inside this one — its address, its
index, what a stranger is shown, what it costs to serve, and the QR on the last
slide that points at it — so it moved whole to
**[`gallery-page.md`](gallery-page.md)**. What is left here is the
PHOTOGRAPHS: where they come from, which ones may be seen, and the controls
over them.
...
### Four bands, and the showcase leads

*"Can we have it so the three showcase photos order themselves to the top as
well?"* — so: **a look**, **the showcase**, **on the gallery**, **the rest**.
A star implies a lamp (`gallery.js` publishes on a star and unstars on a hide,
in one write), so the showcase is a strict subset of the green band and is
never longer than `MAX_PINS`.

**Flagged sits on top of all of it.** A rude photograph is nearly always one the
room sent, so it is nearly always red — and putting red below the fold would
bury exactly what `src/moderation.js` exists to surface. The heading does not
exist at all unless something is flagged.

### And it re-sorts on every press, which reverses the day-old rule

*"As I press green or showcase the order re-arranges on a click basis."*

**Yesterday this said "sorted on load and never again"**, on the reasoning that
a press would slide the photograph out from under the finger that made it. That
is a real effect and it is the wrong conclusion. What the reordering actually
produces is an **inbox**: you work down the reds, each one you promote leaves
the pile, and the band in front of you is always what is still undecided —
which is the job this screen exists for. The stated cost: the tile that shuffles
into a vacated slot is a different photograph, so two fast taps on one spot can
land on the wrong one.

Three things it needed, all of them the sort that fail quietly:

- **The figures are MOVED, never rebuilt.** Each tile carries four listeners and
  two optimistic in-flight writes; re-rendering would drop a lamp mid-request
  and lose whatever it was about to save. `appendChild` on a node already in the
  document moves it, and a detached node keeps its listeners, so a re-lay-out is
  pure reordering.
- **A painter has to write its state back onto the payload.** The lamp already
  did (`p.onGallery`); the star did not. `bandOf()` reads the photograph rather
  than the closure, so without `p.pinned = pinned` the star lit its own corner
  and the grid stayed put — a control reporting success it did not have.
- **And no guard may identify a tile by position.** `community-bay.mjs` pressed
  the first lamp and then read the first lamp, which after a reorder is a
  different photograph: it reported `true -> true` against a working control.
  Every tile carries `data-name` now and the guard follows the photograph. Same
  family as *measure `getClientRects()`, not `querySelectorAll().length`* — a
  check that identifies a thing by where it sits is measuring the layout.
...
## AND WHAT A PHOTOGRAPH DOES ON THE NIGHT IS ITS OWN FILE

Three sections moved whole to
**[`photos-on-the-night.md`](photos-on-the-night.md)** on 16 September 2026,
when this one crossed 100KB and `test/docs-index.test.js` said so: **the camera
photo that starts a night**, **the funniest-photo vote**, and **the second
screen**. Nothing was reworded — they were moved by section, which is the only
kind of split this repo trusts.

**The boundary is a real one rather than a byte count.** This file is the
RECORD: where a photograph is stored, what it costs to serve, who may see it,
and how a night is published. That one is the NIGHT: the ask at the door, the
vote at the break, and the screens the pictures land on. *Gigs is evidence,
Calendar is organisation* — the same test, applied to the photographs
themselves.
...
## WHERE THE BYTES ACTUALLY LIVE IS ITS OWN FILE

**[`photos-in-a-bucket.md`](photos-in-a-bucket.md)** — the move off GitHub onto
an object store, and the evening the bucket was handed the accounts book as well.
Moved out whole on 18 September 2026 because this file reached 99,478 bytes of a
100,000 cap, which is a guard about to fail on whoever writes the next paragraph.

**The boundary is the same one this file already draws twice.** This file is the
RECORD — who may see a photograph, how a night is published, what it costs to
serve. That one is the PLUMBING underneath it: which store the bytes are in, what
a delete means there, and what happened when one word in `inStore()` sent the
accounts book to the same place.

## THE CONTROLS ON A PHOTOGRAPH ARE THEIR OWN FILE

The lamp, the showcase star, the rail's P and the publish control under the
grid — what a quizmaster PRESSES between the gig and the public page — moved
