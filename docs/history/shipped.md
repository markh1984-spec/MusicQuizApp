# What shipped before 21 August 2026

The older half of `docs/history.md`, moved WHOLE on 23 August 2026 when that
file crossed the 100,000-byte cap in `test/docs-index.test.js` — the cap that
exists because `business.md` once reached 170KB with 89% of it under a single
heading, which is precisely the shape `## Current state` had grown into here:
1,500 lines under one heading.

**Split by DATE rather than by subject, deliberately.** Everything in this file
is the same subject — what shipped and what each night found — so there is no
honest subject boundary to cut on; what there IS is a boundary in how often it
is read. The recent entries are consulted; these are looked up. 21 August is
where the current run of console work begins, so nothing being actively
changed is in here.

**Moved by line number, nothing retyped.** The mechanical transform, for the
reason CLAUDE.md gives: a script cannot quietly reword a decision on the way
through.
