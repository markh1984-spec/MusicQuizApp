...
async function handleWrite(req, res, url, route) {
  ...
  /*
   * ---- THE QUIZMASTER'S OWN CAMERA, into TONIGHT'S bucket ----------------
   *
   * Asked for directly: *"have, say, like an app on my phone... that I take
   * photos and it goes into that same bucket from that same evening."*
   *
   * **THERE IS NO NEW APP, AND THAT IS THE POINT.** The control view already
   * IS the thing in his hand all night — signed in, pointed at his own room,
   * open on a phone behind a microphone. A second page to install would be a
   * second thing to remember at the moment there is least room to remember
   * anything, and it would need its own way of knowing which room it is.
   *
   * **IT IS ITS OWN ROUTE BECAUSE IT CARRIES BYTES.** Everything under
   * `/api/host/*` starts with `readJson(req)`, which consumes the body — so a
   * photograph threaded through there would arrive as a parse failure. Same
   * reasoning as the Stripe webhook, one layer down.
   *
   * **NO PLAYER, NO TEAM NAME, NO ROW ON THE BOARD.** `POST /api/photo` needs
   * a joined phone, and joining his own room to take a picture would put the
   * quizmaster on the leaderboard and on the projector — the fault
   * `setOrganiser()` exists to prevent, reached from the other direction.
   * Here the room comes from WHO IS SIGNED IN, the rule every `/api/host/*`
   * route follows, and the photograph lands with an empty `teamName`: the
   * screen only draws a caption when there is one, and a picture of the room
   * captioned with the name of the person who took it is not a caption
   * anybody wanted six feet wide.
   *
   * **ONE BUCKET, WHICH IS THE WHOLE ASK.** It goes through `photos.add()`,
   * so it is on the projector's strip, on `/wall`, in the host's own grid
   * with a bin on it, and filed to the night's folder in the private repo by
   * the same background write as every other photograph. There is no second
   * store and nothing here knows where a picture came from.
   *
   * **THE BREAK PLAN IS NOT CONSULTED, DELIBERATELY.** `photosWanted()` also
   * answers *is the camera being offered to PHONES right now*, which is a
   * question about the gaps in the night and has nothing to do with the
   * person driving it. The kill switch DOES apply — `photos.add()` refuses
   * when the room's photographs are switched off — because a photograph
   * accepted into a store the screen is ignoring is one that vanishes with no
   * explanation.
   */
  if (route === '/api/host/photo' && req.method === 'POST') {
    const room = roomForHost(req, url);
    ...
    const result = room.photos.add(bytes, {
      ...
      playerId: '',
      teamName: '',
      // Read on the raw file before the browser's own canvas redraw stripped
      // the EXIF — see looksCameraTaken() in filters.js. Never a gate; it only
      // decides whether this one is eligible for the public gallery later,
      // which for a quizmaster's own photograph is usually the whole reason
      // they took it.
      camera: url.searchParams.get('camera') !== '0',
    });
    if (result.ok) pushState(room);
