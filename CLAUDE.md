...
| **"Filters" means PROPS, and the colour grading is GONE** | `public/assets/stickers.js` — two dozen of them now, drawn like the seasonal motifs and for the same reason. The host asked for "clown noses, dog ears etc." and found colour grading, which is what `filters.js` does; that was folded away behind "Change the colour instead" and is now **deleted from the panel entirely** — *"I want funny props to be the focus"*. `drawFiltered(…, 'none')` stays as the DRAW path, because it is the one place the sizing is worked out and the preview and the upload go through it so they cannot drift. **No face detection anywhere**: a model is megabytes on a stranger's phone over pub wifi and `FaceDetector` does not exist on iOS Safari — both break *no dependencies* and both fail on somebody's handset in a room. **A prop is dragged straight off its tile**, not tapped and then dragged: `pointerdown` on a tile creates it and hands it to the drag code, and move/up listen on the WINDOW so one gesture crosses two elements. A tap still centres it — nothing moves until the thumb has travelled 8px, so a tap and a drag are told apart by the finger rather than by a mode. **A bin appears only while something is in the air**, bottom right, furthest from where a face is; drop on it to take that ONE prop off, where "Take it off" still removes the last added. Positions are stored as a **fraction** of the canvas, never pixels. Tiles are mid-grey (half the props are nearly black) and **named as well as drawn** — unlike the colour looks, which were shown rather than named because a name told you nothing; at 50px a monobrow and a moustache are the same dark smear. |
