...
**[`docs/console/chrome.md`](docs/console/chrome.md)** — moved there whole on 9
September 2026 to pay for the Stripe section. **Read it before touching the
...
Every one lived in a band no guard looked at. The measurements are in
...
- **AND THE FIFTH IS THE ONE WRITER THAT NEVER WENT THROUGH IT —
  `galleryRoomOf()`.** Every photo ROUTE resolves through `galleryRoomFor()`;
...
- **THE GALLERY GATE IS THE SOURCE — the house camera's photos show, the room's
  wait for a green lamp** (`showsByDefault()` = `isHousePhoto(name)`, marked from
  `playerId`: `/api/snap` has none, a punter always does). It replaced an EXIF
  guess that was wrong INCONSISTENTLY. **`showsOnGallery()` is the ONE decision.**
...
- **A GLOBAL "last played" ANSWERS THE WRONG QUESTION** — *have I run this
  lately* rather than *will this room have heard it*, so the 80s quiz run at
  The Crown on Tuesday is fresh at The Station Tap and was buried at both.
...
`src/stripe.js` — the one file that knows the word — plus
`/api/stripe/webhook`, `/api/subscribe`, `/api/billing/portal` and
`console-subscribe.js`. `billing.js` did not change: that is what its
processor-agnosticism was for. **A SEPARATE STRIPE ACCOUNT under the same
login, never a second business inside the kids'-party one** — the statement
descriptor, the branding, the receipts and the payouts are all per account.

- **NO SDK** — form-encoded HTTP and an HMAC node's own `crypto` computes.
- **CHECKOUT, NEVER A CARD FORM HERE.** A number that never touches this
  server is one it can never leak.
- **THE ACCOUNT ID TRAVELS ON THE SUBSCRIPTION, not only the session** — a
  renewal months later has no session on it.
- **THE TIER IS READ OFF THE PRICE, never off the request** — `wantedTier`
  must never become `tier`. **An unknown price grants nothing.**
- **THE WEBHOOK IS FIRST IN `handleWrite()` BECAUSE IT NEEDS THE RAW BYTES**,
  and **it answers 200 to almost everything**: one that errors on an event it
  does not care about gets disabled by Stripe, silently. 400 is for a failed
  signature alone.
- **`customer.subscription.updated` IS NOT HANDLED** — it fires for a card
  update and a proration, which are not about whether anybody has paid.
- **CANCELLING IS STRIPE'S PORTAL, NOT A BUTTON HERE** — a second place a
  subscription can end is a second place it can disagree with the first.
- **A GROUP SEAT CANNOT SUBSCRIBE** — its standing is its parent's.
- **`console-tiers.js` MAY NOT TOUCH THE NETWORK** — there is a guard. It
