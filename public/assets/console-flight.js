...
export function flightPanel() {
  const el = node(`
    <div class="panel flight-panel">
      ...
      <div class="tiny">The server's own record of this room: the boot, its self-test, every refusal and what each screen reported. Paste it into the chat and the fault is usually named on the first read.</div>
  ...
  async function refresh() {
    try {
      const res = await fetch('/api/flight', { cache: 'no-store' });
  ...
  el.querySelector('.flight-copy').addEventListener('click', async () => {
    const stamp = `Quizporium — what the app saw, ${new Date().toLocaleString('en-GB')}, ${location.host}\n${self.textContent}\n\n`;
    try {
