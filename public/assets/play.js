...
function openCamera() {
  openCameraSheet({
     ...
     * screen, which was complete until the public gallery shipped and then
     * quietly was not: a night can be published, and the person holding the
     * phone is the one who should hear that first. Naming WHO decides matters
    ...
    ready: () => Boolean(me),
    async send({ blob, filter, camera, shown, used }) {
      const tally = `&shown=${encodeURIComponent(shown.join(','))}&used=${encodeURIComponent(used.join(','))}`;
      const res = await fetch(`/api/photo?playerId=${encodeURIComponent(me.id)}&filter=${encodeURIComponent(filter)}${camera ? '&camera=1' : ''}${tally}${roomParam()}`, {
        method: 'POST',
